...
| One-time review handoff is policy-bound | Passed deterministically and in local and public-origin native Chrome | Proposal produces `review_required` without changing the release; exact approval enables one execution; wrong request, expiry, revocation, rejection, and replay are denied by the shared policy. |
| Tool surface follows review state | Passed deterministically and in local and public-origin native Chrome | The lifecycle suite and native runs prove proposal registration only after stabilisation, removal while review is pending, execution-tool registration after approval, and removal after execution or revocation. |
...
| Native one-time review handoff | Passed on local and public-origin Chrome | After the service became stable, the proposal tool returned `review_required` and request `TWA-REVIEW-0001` without changing the release. Human approval registered `execute_approved_checkout_fix`; native execution changed the visible release, returned `approvalConsumed: true`, and removed that tool. |
| Native tool result survives state-aware unregistration | Passed on local and public-origin Chrome | The first native run exposed a lifecycle race: removing the proposal tool during its own invocation caused a transient browser error after the state changed. Registration sync is now deferred until the invocation result resolves; both reruns returned the complete structured review result before removing the tool. |
...
| Public repository and deployment exist | Passed | Public source: [markneville/trusted-webmcp-actions](https://github.com/markneville/trusted-webmcp-actions). HTTPS deployment: [Trusted WebMCP Actions](https://markneville.github.io/trusted-webmcp-actions/). GitHub Pages workflow run `32920236541` deployed commit `fba0e4f`. |
| Native enforcement works on the public HTTPS origin | Passed | Chrome reported `Native WebMCP` on the public site. Native inspect returned the seeded incident; a 20% shift was allowed with `uiUpdated: true`; proposal and approval exposed the exact one-time execution path; execution changed the release and consumed the approval; revocation removed all governed tools while retaining `inspect_incident`. |
...
- The local and public-origin native Chrome lifecycles pass proposal → review → approval → one-time execution. On the public origin, receipts `TWA-REF-0002` and `TWA-REF-0003` proved review-required and allowed decisions, the release changed exactly once, the execution tool disappeared, and revocation left only `inspect_incident`.
